...
class Resize:
    ...
    def resize_image(self, image, scale):
        ...
        image = cv2.copyMakeBorder(image, 0, self.target_size[0] - resize_h,
                                   0, self.target_size[1] - resize_w, cv2.BORDER_CONSTANT)  # top, bottom, left, right
        return image
    ...
    def resize_label(self, label, scale):
        ...
        label = cv2.copyMakeBorder(label, 0, self.target_size[0] - resize_h,
                                   0, self.target_size[1] - resize_w, self.ignore_label)  # top, bottom, left, right
        return label
...
class RandomResizeCrop:
    def __init__(self, base_size=2048, crop_size=(512, 1024), downsample_rate=1,
                 scale_factor=16, keep_ratio=True, interp=None, ignore_label=255, multi_scale=False):
        """
        Resize image to target size. if keep_ratio is True,
        resize the image's long side to the maximum of target_size
        if keep_ratio is False, resize the image to target size(h, w)
