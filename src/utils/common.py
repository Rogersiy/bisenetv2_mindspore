...
def init_env(cfg):
    ...
    if cfg.rank % min(cfg.rank_size, 8) == 0:
        ...
        if not os.path.exists(cfg.save_dir):
            ...
            os.makedirs(os.path.join(cfg.save_dir, "checkpoints"), exist_ok=True)
    # Set Logger
    logger.setup_logging(logger_name="BiSeNetv2", log_level="INFO", rank_id=rank, device_per_servers=rank_size)
    ...
    if cfg.enable_modelarts:
        ...
        if cfg.ckpt_url:
            ...
            sync_data(cfg.ckpt_url, os.path.join(cfg.ckpt_dir, "finetune500epoch.ckpt"))  # pretrain ckpt
